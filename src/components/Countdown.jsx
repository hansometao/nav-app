    ...
    if (showDays) {
      return (
        <div className="countdown-time">
          ...
          <span className="cd-num">
            {String(tl.h).padStart(2, '0')}
            <small>时</small>
          </span>
          <span className="cd-sep">:</span>
          <span className="cd-num">
            {String(tl.m).padStart(2, '0')}
            <small>分</small>
          </span>
          <span className="cd-sep">:</span>
          <span className="cd-num cd-sec">
  ...
  return (
    <div className="widget countdown-widget">
      <div className="widget-header">
        <h3>
          <Icon name="clock" size={18} /> 倒计时
        </h3>
        <button className="btn-icon" onClick={() => setShowAdd(!showAdd)} title="添加倒计时">
      ...
      {showAdd && (
        <div className="add-event-form countdown-add-form">
              ...
              <div className="time-inputs-row">
                <div className="time-input-group">
                  ...
                  <input
                    ...
                    onChange={e =>
                      setNewCountdown({ ...newCountdown, hour: Number(e.target.value) })
                    }
                  />
                </div>
                <div className="time-input-group">
                  ...
                  <input
                    ...
                    onChange={e =>
                      setNewCountdown({ ...newCountdown, minute: Number(e.target.value) })
                    }
                  />
                </div>
              </div>
              <div className="selected-time-preview">
                设定时间：{String(newCountdown.hour).padStart(2, '0')}:
                {String(newCountdown.minute).padStart(2, '0')}
              </div>
            </>
          )}
          ...
          <button className="btn-sm" onClick={addCountdown}>
            添加
          </button>
        </div>
      )}
          ...
          return (
            <div key={cd.id} className="countdown-item" style={{ borderLeftColor: cd.color }}>
              <div className="countdown-label">
                ...
                {cd.label}
                <button className="btn-icon btn-remove" onClick={() => removeCountdown(cd.id)}>
                  <Icon name="x" size={14} />
                </button>
              </div>
              {tl.expired ? (
      ...
      <div className="timer-section">
        ...
        <div className="timer-controls">
          ...
          />
          {!timerRunning ? (
            <button className="btn-sm btn-success" onClick={startTimer}>
              开始
            </button>
          ) : (
            <button className="btn-sm btn-danger" onClick={stopTimer}>
              停止
            </button>
          )}
        </div>
        {timerRunning && <div className="timer-display">{formatTimer(timer)}</div>}
      </div>
    </div>
  );
}
